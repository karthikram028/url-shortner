...
function HandleRedirectContainer() {
  ...
  useEffect(() => {
    async function getData() {
        return axios.get(`${SERVER_ENDPOINT}/api/url/${shortId}`).then((res) => setDestination(res.data.destination)).catch((error) => {
            setError(error.message);});}
    getData();
  }, [shortId]);

  useEffect(() => {
    if (destination) {window.location.replace(destination);}}, [destination]);
